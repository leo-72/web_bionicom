...
function sendMessage() {
  ...
  // Menggunakan encodeURIComponent untuk mengonversi teks ke dalam URL yang valid
  var encodedMessage = encodeURIComponent(whatsappMessage);
  
  // Nomor WhatsApp yang akan menerima pesan
  var phoneNumber = '62821398760827'; // Ganti dengan nomor WhatsApp yang sesuai
  // Membuat URL untuk redirect ke WhatsApp dengan pesan yang sudah dibuat
  var whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
  // Melakukan redirect ke WhatsApp
  window.location.href = whatsappURL;
}
// Menangani pengiriman pesan saat tombol "Send" diklik
document.querySelector('form').addEventListener('submit', function(event) {
  event.preventDefault(); // Menghentikan perilaku bawaan formulir (submit)
  sendMessage(); // Memanggil fungsi untuk mengirim pesan ke WhatsApp
});